...
from infinite_craft_cli.client import InfiniteCraftClient, _BASE_URL, _PAIR_ENDPOINT, _IMPERSONATE
...
def _make_mock_session(response_data=None):
    ...
    mock_resp = MagicMock()
    mock_resp.json.return_value = response_data or {"result": "Steam", "emoji": "💨", "isNew": False}
...
class TestSessionLifecycle:
    def test_visits_base_url_on_enter(self):
        ...
        assert first_call.kwargs.get("url") == _BASE_URL or first_call.args[0] == _BASE_URL
...
class TestPair:
    ...
    def test_returns_element_on_success(self):
        mock_cls, _, _ = _make_mock_session({"result": "Steam", "emoji": "💨", "isNew": False})
        with patch("infinite_craft_cli.client.RateLimiter") as MockRL:
            MockRL.return_value.acquire = AsyncMock()
            with patch("curl_cffi.requests.AsyncSession", mock_cls):
                client = InfiniteCraftClient()
    ...
    def test_returns_none_element_for_nothing(self):
        mock_cls, _, _ = _make_mock_session({"result": "Nothing", "emoji": "", "isNew": False})
        with patch("infinite_craft_cli.client.RateLimiter") as MockRL:
            MockRL.return_value.acquire = AsyncMock()
            with patch("curl_cffi.requests.AsyncSession", mock_cls):
                client = InfiniteCraftClient()
    ...
    def test_first_discovery(self):
        mock_cls, _, _ = _make_mock_session({"result": "Unicorn", "emoji": "🦄", "isNew": True})
        with patch("infinite_craft_cli.client.RateLimiter") as MockRL:
            MockRL.return_value.acquire = AsyncMock()
            with patch("curl_cffi.requests.AsyncSession", mock_cls):
                client = InfiniteCraftClient()
