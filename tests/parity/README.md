...
| `prioritize_pairs` | `{"pairs": [6-tuples], "recipes": {...}, "cached": ["a\0b", ...]}` | same 6-tuple list, reordered | Batch execution order: ingredient-usage score (sum) descending, ascending pair-key tie-break; then pairs whose NUL-joined canonical key is in `cached` are promoted to the front (stable within hits/misses). |
